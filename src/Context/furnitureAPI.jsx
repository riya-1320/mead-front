...
export const MaterialsProvider = ({ children }) => {
  ...
  const fetchMaterialTypeOptions = async () => {
    ...
    try {
      const response = await axios.get('https://13.202.225.45:5000/api/components/rate-materials', {
        headers: {
  ...
  const addMaterial = async (materialType, options) => {
    ...
    try {
      const response = await axios.post(
        'https://13.202.225.45:5000/api/components/rate-materials',
        {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch('https://13.202.225.45:5000/api/components/user/alluser',{
        headers: {
          'Content-Type': 'application/json',
  ...
  const updateUser = async (id, userData) => {
    setLoading(true);
    try {
      const response = await fetch(`https://13.202.225.45:5000/api/components/user/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'x-auth-token': token,
...
const addUser = async (userData) => {
  setLoading(true);
  try {
    const response = await fetch('https://13.202.225.45:5000/api/components/register', { // Adjust endpoint as necessary
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-auth-token': token,
